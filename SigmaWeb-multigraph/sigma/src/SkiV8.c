...
Run Mode:           HI_SPEED
Trace Vars:         Q[0],Q[1],Q[2]
Random Number Seed: 12345
Initial Values:     3,120,180,10,12,8,8,6,6,6,6,6,.5,.5,0,.5,.5,0,1,0,0,.1,.1,.1,2,2,2,2,3,3,1,1,1,1,1,1,.3,.3,1000
Ending Condition:   STOP_ON_TIME
Ending Time:        240.000
Trace Events:       RUN,START,ENTER
Hide Edges:         
...
long   Q[3];	/***  Queue size for ski lifts  ***/
long   B[3];	/***  Batch size for ski lifts  ***/
double TA[3][2];	/***  New skiier arrival time distribution  ***/
double LAMBDA[3];	/***  Chair interarrival times  ***/
double TS[3];	/***  Ski lift time of service  ***/
double SKI[3][3];	/***  Lift to lift ski time  ***/
...
double PR[3][3];	/***  Probability transition matrix  ***/
long   N;	/***  Number of lifts  ***/
long   CLOSE;	/***  Time when no more skiiers enter system  ***/
long   LEAVE;	/***  Time when skiiers begin leaving resort  ***/
double MULT[3];	/***  Beta distribution multiplier  ***/
double ALPHA[3];	/***  Alphas for beta dist  ***/
double BETA[3];	/***  Betas for Beta dist  ***/
...
char y_n = 'p';            /* yes/no for file overwrite*/
  ...
  if (fscanf(input_fp,"%s %1s %ld %lf %d", output_file_name, &y_n, &rndsd, &stop_time, &trace_flag)<4
     ...
     || fscanf(input_fp,"%ld", &CLOSE)<1
     || fscanf(input_fp,"%ld", &LEAVE)<1
     || fscanf(input_fp,"%lf", &SKI[0][0])<1
     || fscanf(input_fp,"%lf", &SKI[0][1])<1
     || fscanf(input_fp,"%lf", &SKI[0][2])<1
     || fscanf(input_fp,"%lf", &SKI[1][0])<1
     || fscanf(input_fp,"%lf", &SKI[1][1])<1
     || fscanf(input_fp,"%lf", &SKI[1][2])<1
     || fscanf(input_fp,"%lf", &SKI[2][0])<1
     || fscanf(input_fp,"%lf", &SKI[2][1])<1
     || fscanf(input_fp,"%lf", &SKI[2][2])<1
     || fscanf(input_fp,"%lf", &PR[0][0])<1
     || fscanf(input_fp,"%lf", &PR[0][1])<1
     || fscanf(input_fp,"%lf", &PR[0][2])<1
     || fscanf(input_fp,"%lf", &PR[1][0])<1
     || fscanf(input_fp,"%lf", &PR[1][1])<1
     || fscanf(input_fp,"%lf", &PR[1][2])<1
     || fscanf(input_fp,"%lf", &PR[2][0])<1
     || fscanf(input_fp,"%lf", &PR[2][1])<1
     || fscanf(input_fp,"%lf", &PR[2][2])<1
     || fscanf(input_fp,"%lf", &LAMBDA[0])<1
     || fscanf(input_fp,"%lf", &LAMBDA[1])<1
     || fscanf(input_fp,"%lf", &LAMBDA[2])<1
     || fscanf(input_fp,"%ld", &B[0])<1
     || fscanf(input_fp,"%ld", &B[1])<1
     || fscanf(input_fp,"%ld", &B[2])<1
     || fscanf(input_fp,"%lf", &TS[0])<1
     || fscanf(input_fp,"%lf", &TS[1])<1
     || fscanf(input_fp,"%lf", &TS[2])<1
     || fscanf(input_fp,"%lf", &ALPHA[0])<1
     || fscanf(input_fp,"%lf", &ALPHA[1])<1
     || fscanf(input_fp,"%lf", &ALPHA[2])<1
     || fscanf(input_fp,"%lf", &BETA[0])<1
     || fscanf(input_fp,"%lf", &BETA[1])<1
     || fscanf(input_fp,"%lf", &BETA[2])<1
     || fscanf(input_fp,"%lf", &MULT[0])<1
     || fscanf(input_fp,"%lf", &MULT[1])<1
     || fscanf(input_fp,"%lf", &MULT[2])<1
     ) {
...
if (y_n == 'n' || y_n == 'N') {
  ...
  if (trace_flag != 0 && trace_flag != 1) {
    ...
    printf("Not found, input data at the keyboard.\n");
     ...
     scanf  ( "%ld", &N);
     printf ( "\nEnter initial value for CLOSE: \n");
     scanf  ( "%ld", &CLOSE);
     printf ( "\nEnter initial value for LEAVE: \n");
     scanf  ( "%ld", &LEAVE);
     printf ( "\nEnter initial value for SKI[0][0]: \n");
     scanf  ( "%lf", &SKI[0][0]);
     printf ( "\nEnter initial value for SKI[0][1]: \n");
     scanf  ( "%lf", &SKI[0][1]);
     printf ( "\nEnter initial value for SKI[0][2]: \n");
     scanf  ( "%lf", &SKI[0][2]);
     printf ( "\nEnter initial value for SKI[1][0]: \n");
     scanf  ( "%lf", &SKI[1][0]);
     printf ( "\nEnter initial value for SKI[1][1]: \n");
     scanf  ( "%lf", &SKI[1][1]);
     printf ( "\nEnter initial value for SKI[1][2]: \n");
     scanf  ( "%lf", &SKI[1][2]);
     printf ( "\nEnter initial value for SKI[2][0]: \n");
     scanf  ( "%lf", &SKI[2][0]);
     printf ( "\nEnter initial value for SKI[2][1]: \n");
     scanf  ( "%lf", &SKI[2][1]);
     printf ( "\nEnter initial value for SKI[2][2]: \n");
     scanf  ( "%lf", &SKI[2][2]);
     printf ( "\nEnter initial value for PR[0][0]: \n");
     scanf  ( "%lf", &PR[0][0]);
     printf ( "\nEnter initial value for PR[0][1]: \n");
     scanf  ( "%lf", &PR[0][1]);
     printf ( "\nEnter initial value for PR[0][2]: \n");
     scanf  ( "%lf", &PR[0][2]);
     printf ( "\nEnter initial value for PR[1][0]: \n");
     scanf  ( "%lf", &PR[1][0]);
     printf ( "\nEnter initial value for PR[1][1]: \n");
     scanf  ( "%lf", &PR[1][1]);
     printf ( "\nEnter initial value for PR[1][2]: \n");
     scanf  ( "%lf", &PR[1][2]);
     printf ( "\nEnter initial value for PR[2][0]: \n");
     scanf  ( "%lf", &PR[2][0]);
     printf ( "\nEnter initial value for PR[2][1]: \n");
     scanf  ( "%lf", &PR[2][1]);
     printf ( "\nEnter initial value for PR[2][2]: \n");
     scanf  ( "%lf", &PR[2][2]);
     printf ( "\nEnter initial value for LAMBDA[0]: \n");
     scanf  ( "%lf", &LAMBDA[0]);
     printf ( "\nEnter initial value for LAMBDA[1]: \n");
     scanf  ( "%lf", &LAMBDA[1]);
     printf ( "\nEnter initial value for LAMBDA[2]: \n");
     scanf  ( "%lf", &LAMBDA[2]);
     printf ( "\nEnter initial value for B[0]: \n");
     scanf  ( "%ld", &B[0]);
     printf ( "\nEnter initial value for B[1]: \n");
     scanf  ( "%ld", &B[1]);
     printf ( "\nEnter initial value for B[2]: \n");
     scanf  ( "%ld", &B[2]);
     printf ( "\nEnter initial value for TS[0]: \n");
     scanf  ( "%lf", &TS[0]);
     printf ( "\nEnter initial value for TS[1]: \n");
     scanf  ( "%lf", &TS[1]);
     printf ( "\nEnter initial value for TS[2]: \n");
     scanf  ( "%lf", &TS[2]);
     printf ( "\nEnter initial value for ALPHA[0]: \n");
     scanf  ( "%lf", &ALPHA[0]);
     printf ( "\nEnter initial value for ALPHA[1]: \n");
     scanf  ( "%lf", &ALPHA[1]);
     printf ( "\nEnter initial value for ALPHA[2]: \n");
     scanf  ( "%lf", &ALPHA[2]);
     printf ( "\nEnter initial value for BETA[0]: \n");
     scanf  ( "%lf", &BETA[0]);
     printf ( "\nEnter initial value for BETA[1]: \n");
     scanf  ( "%lf", &BETA[1]);
     printf ( "\nEnter initial value for BETA[2]: \n");
     scanf  ( "%lf", &BETA[2]);
     printf ( "\nEnter initial value for MULT[0]: \n");
     scanf  ( "%lf", &MULT[0]);
     printf ( "\nEnter initial value for MULT[1]: \n");
     scanf  ( "%lf", &MULT[1]);
     printf ( "\nEnter initial value for MULT[2]: \n");
     scanf  ( "%lf", &MULT[2]);
   }
...
int  _edge_condition[3];
  ...
  /* state changes */
  P=RND;
  NEXTRIDE=(P>PR[I][0])+(P>(PR[I][0]+PR[I][1]));
...
int  _edge_condition[1];
  ...
  if (_edge_condition[0])
    ...
    transfer[3] = I;
    for ( t_index=4; t_index<maxatr; t_index++) transfer[t_index] = 0.0;
    event_time = current_time + MULT[I]*BET(ALPHA[I],BETA[I]);
    event_type = ENTER_event;
    event_priority = 5;
    schedule_event();
    }
